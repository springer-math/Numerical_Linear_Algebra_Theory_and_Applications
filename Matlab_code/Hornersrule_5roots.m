% Polynomial p(x)=(x-1)^2(x-2)(x-3)(x-4)(x-5)
...
%exact coefficients of polynomial p(x) = (x -1)^2(x-2)(x-3)(x-4)(x-5)

%input interval for  p(x): can be changed by user
...
a=double(coeffs((t-1).^2*(t-2)*(t-3)*(t-4)*(t-5),t))
...
for i = 1:N
  ...
  ComputedErrors(i) = P- (x(i)-1).^2*(x(i)-2)*(x(i)-3)*(x(i)-4)*(x(i)-5);
  y(i) =  (x(i)-1).^2*(x(i)-2)*(x(i)-3)*(x(i)-4)*(x(i)-5);
